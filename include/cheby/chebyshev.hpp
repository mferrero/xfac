...
//-------------------------------------------------------- Fast Discreat Cosine Transform ---------------------------------
...
{
    uint N = x.size();
    if (N==1) return arma::vec({1.});
    arma::vec x_prepare(N, arma::fill::zeros);
    for (uint k =0; k <= N/2-1; k++) x_prepare.at(k) = x.at(2*k);
    for (uint k =0; k <= N/2-1; k++) x_prepare.at(N-k-1) = x.at(2*k+1);
    if (N&1) x_prepare.at((N-1)/2) = x.at(N-1);
    arma::cx_vec y = arma::fft(x_prepare);
    arma::vec Y (N);
    for (uint k=0;k<N;k++) Y.at(k) = cos(k*M_PI/(2*N))*y.at(k).real()+sin(k*M_PI/(2*N))*y.at(k).imag();
    return Y;
}


inline arma::cube DCT_matrix_(arma::cube const& x)
// implementation of type II DCT for doubles using arma objects
{
    uint N = x.n_slices;
    if (N==1) return arma::cube(x.n_rows, x.n_cols, x.n_slices, arma::fill::ones);
    arma::cube x_prepare(x.n_rows, x.n_cols, x.n_slices);
    arma::cube Y(x.n_rows, x.n_cols, x.n_slices);
    for (uint k =0; k <= N/2-1; k++) x_prepare.slice(k) = x.slice(2*k);
    for (uint k =0; k <= N/2-1; k++) x_prepare.slice(N-k-1) = x.slice(2*k+1);
    if (N&1) x_prepare.slice((N-1)/2) = x.slice(N-1); // even or odd, identical to if(n%2==1)
    ...
    for (uint k=0;k<N;k++) Y.slice(k) = cos(k*M_PI/(2*N))*arma::real(y.slice(k))+sin(k*M_PI/(2*N))*arma::imag(y.slice(k));
    return Y;
}
...
// implementation of type II DCT for complex using arma objects
{
   ...
   arma::cube  X_real = DCT_matrix_(arma::real(x));
   arma::cube  X_imag = DCT_matrix_(arma::imag(x));
...
inline std::vector<double> calc_cheby_abscissas(double a, double b, int n) {
    ...
    if (a >= b) throw std::invalid_argument("calc_cheby_abscissas: a >= b");
    if (n < 1) throw std::invalid_argument("calc_cheby_abscissas: n < 1");
...
// Pimped implementation from Numerical Recipes 3. edition.
 public:
    ...
    T operator() (double x, int m) const {
        ...
        double y, y2;
        T d=0.0, dd=0.0, sv;
        if ((x-a)*(x-b) > 0.0) throw std::invalid_argument("x not in range in Chebyshev call");
        if (m > n || m < 1) throw std::invalid_argument("m not correct");
    ...
    void set_m(int m_) {
        if (m_ > n || m_ < 1) throw std::invalid_argument("m not correct");
        m = m_;
    }
...
inline std::vector<double> cheby_weights(double x, double a, double b, int n, int deriv=0, int m=0) {
    ...
    for(int i=0; i<n; i++)
    {
        auto f=[x0=xi[i]](double x){if (x==x0) return 1.0; else return 0.0;};
        switch (deriv){
            case 0:
...
public:
    ...
    void addInterval(std::vector<T> fi, double a , double b){
        ...
        if (n < 1) throw std::invalid_argument("Cheby::addInterval: n < 1");
        if (a >= b) throw std::invalid_argument("Cheby::addInterval: a >= b");
        if (std::count(av.begin(), av.end(), a)) throw std::invalid_argument("Cheby::addInterval: interval bound a already present");
        if (std::count(bv.begin(), bv.end(), b)) throw std::invalid_argument("Cheby::addInterval: interval bound b already present");
...
class ChebyMat{
 ...
 public:
    ...
    ChebyMat(std::vector<std::tuple<double, double, size_t>> grid)
     : _grid{grid}
     {
        ...
        */
        ...
        if (grid.size() == 0) throw std::invalid_argument("ChebyMat::init: size is zero");
        if (grid.size() > 1) throw std::invalid_argument("ChebyMat::init: only single grid implemented");  // DEBUG workaround
    ...
    void interpolate(std::function<arma::Mat<T>(double)> func) {
        auto fi = func(xi[n-1]);
        arma::cx_cube fx(fi.n_rows, fi.n_cols, n);
        fx.slice(0) = fi;
        for (int i = 1; i < n; i++)
    ...
    arma::Mat<T> convolute_left(std::function<arma::Mat<T>(double)> func, double x) const {
        ...
        auto kernel = [&] (double y) {arma::Mat<T> res = func(x - y) * eval(y); return res;};
    ...
    void convolute_inplace_left(std::function<arma::Mat<T>(double)> func) {
        ...
        auto kernel = [&] (double x) {return convolute_left(func, x);};
        cm.interpolate(kernel);
        c = cm.c;
    }
